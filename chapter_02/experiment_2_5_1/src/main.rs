...
fn load_csv(csv_path: &str, n_features: usize) -> Result<(Tensor, Tensor), Box<dyn Error>> {
    ...
    for line in lines {
        ...
        let values: Vec<f32> = line
            .split(',')
            .map(|s| s.parse::<f32>().unwrap_or(0.0)) // Default to 0.0 if parsing fails
            .collect();
    ...
    let label_tensor = Tensor::from_slice(&labels).to_kind(Kind::Float).to(Device::Cpu).unsqueeze(-1); // Add batch dimension
...
impl nn::Module for Model {
    fn forward(&self, xs: &Tensor) -> Tensor {
        ...
        logits // No sigmoid as BCEWithLogitsLoss handles this internally
    }
}
...
fn train_model(train_features: Tensor, train_labels: Tensor, output_path: &str) -> Result<(), Box<dyn Error>> {
    ...
    for epoch in 1..=num_epochs {
        ...
        for batch_start in (0..train_features.size()[0]).step_by(batch_size) {
            ...
            let logits = model.forward(&batch_xs).squeeze(); // Remove the -1 argument
            let loss = logits.binary_cross_entropy_with_logits::<Tensor>(&batch_ys.squeeze(), None, None, Reduction::Mean); // Remove the -1 argument
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let csv_path = "/home/zack/rust_genomics/gene_expression.csv";
    let output_path = "/home/zack/rust_genomics/output.txt";
